...
====================================================
Versión 3.0 — Diseño pastel moderno con visualizaciones WOW.
- Paleta pastel sin tonos café
...
# Importar utilities
from utils import (
    C, PROD_COLORS, PROD_COLORS_DARK, PROD_LABELS, REC_COLORS, REC_LABELS,
    hex_rgba, get_util_color, get_cobertura_color,
    PLOT_CFG, GRID_CFG, AXIS_CFG, LEGEND_CFG, MARGIN_CFG,
    apply_pastel_style, apply_bar_style, apply_scatter_style,
    calcular_tendencias, generar_resumen_mensual
)
...
/* Header Hero */
.hero {
    background: linear-gradient(135deg, #3D2B1F 0%, #6B5B4F 50%, #8B7355 100%);
...
/* Info boxes */
.info-box {
    background: linear-gradient(135deg, rgba(249,228,160,0.25), rgba(255,248,240,0.7));
    border: 1px solid rgba(244,167,185,0.4);
    border-left: 4px solid #C9B8E8;
    border-radius: 12px;
    padding: 1rem 1.2rem;
    font-size: 0.88rem;
...
with st.sidebar:
    ...
    with st.expander("Configurar modelo LP"):
        ct = st.number_input("Costo prod/und (Ct)", value=4_310, step=100)
...
# ══════════════════════════════════════════════════════════════════════════════
params_custom = {
    **PARAMS_DEFAULT, "Ct": ct, "CRt": crt, "COt": cot, "Ht": ht, "PIt": pit,
...
kpi_card(k4, "⚙️", f"{util_max:.0f}%", "Util. Máx.",
         "Cuello botella" if util_max >= 80 else "✓ OK")
kpi_card(k5, "🌡️", f"{temp_avg:.0f}°C", "Temp. Horno",
         f"Excesos: {excesos}" if excesos else "✓ Sin excesos")
...
with tabs[0]:
    ...
    # Gráfica de líneas con pronóstico
    fig_pro = go.Figure()
    for p in PRODUCTOS:
        ...
        fig_pro.add_trace(go.Scatter(
            x=MESES_ES, y=serie, mode="lines+markers",
            name=PROD_LABELS[p],
            line=dict(color=PROD_COLORS_DARK[p], width=2.5),
            marker=dict(size=8, color=PROD_COLORS[p], line=dict(color="white", width=2)),
        ))
        ...
        fig_pro.add_trace(go.Scatter(
            ...
            marker=dict(size=10, symbol="diamond", color=PROD_COLORS[p]),
            name=f"{PROD_LABELS[p]} (pronóstico)",
            showlegend=False
        ))

    fig_pro.add_vline(
        x=len(MESES_ES) - 1, line_dash="dash", line_color=C["lavender"],
        annotation_text="Pronóstico →", annotation_font_color=C["lavender"]
    )
    fig_pro.update_layout(
        **PLOT_CFG, height=420,
        title="Demanda Histórica y Proyección — Dora del Hoyo",
        xaxis_title="Mes", yaxis_title="Unidades",
        legend=dict(orientation="h", y=-0.22, x=0.5, xanchor="center"),
        xaxis=dict(showgrid=True, gridcolor="#F0E8E0"),
        yaxis=dict(showgrid=True, gridcolor="#F0E8E0"),
    )
    ...
    with col_a:
        ...
        fig_heat = go.Figure(go.Heatmap(
            ...
            colorscale=[[0, "#FFF8F0"], [0.3, "#F9E4A0"], [0.65, "#F4A7B9"], [1, "#C9B8E8"]],
            hovertemplate="%{y}<br>%{x}: %{z:.0f} und<extra></extra>",
            text=[[f"{int(v)}" for v in row] for row in z],
            texttemplate="%{text}", textfont=dict(size=9, color="#3D2B1F"),
        ))
    ...
    with col_b:
        ...
        fig_pie.update_layout(
            **PLOT_CFG, height=280, margin=dict(t=10, b=10),
            annotations=[dict(text="<b>Ventas</b><br>anuales", x=0.5, y=0.5,
                             font=dict(size=11, color="#3D2B1F"), showarrow=False)],
    ...
    fig_hh.add_trace(go.Bar(
        ...
        marker_color=colores_hh, marker_line_color="white", marker_line_width=2,
        hovertemplate="%{x}: %{y:.1f} H-H<extra></extra>", showlegend=False
    ))
    fig_hh.add_trace(go.Scatter(
        x=MESES_ES, y=list(dem_h_vals.values()), mode="lines+markers",
        line=dict(color=C["coral"], width=2.5), marker=dict(size=7),
        showlegend=False
    ))
    fig_hh.update_layout(**PLOT_CFG, height=280, xaxis_title="Mes", yaxis_title="H-H",
                         xaxis=dict(showgrid=False), yaxis=dict(gridcolor="#F0E8E0"))
...
with tabs[1]:
    ...
    fig_agr.add_trace(go.Bar(
        x=df_agr["Mes_ES"], y=df_agr["Inv_Ini_HH"], name="Inv. Inicial H-H",
        marker_color=C["sky"], opacity=0.85, marker_line_color="white", marker_line_width=1
    ))
    fig_agr.add_trace(go.Bar(
        x=df_agr["Mes_ES"], y=df_agr["Produccion_HH"], name="Producción H-H",
        marker_color=C["butter"], opacity=0.9, marker_line_color="white", marker_line_width=1
    ))
    fig_agr.add_trace(go.Scatter(
        x=df_agr["Mes_ES"], y=df_agr["Demanda_HH"], mode="lines+markers",
        name="Demanda H-H",
        line=dict(color=C["coral"], dash="dash", width=2.5),
        marker=dict(size=8, color=C["coral"])
    ))
    fig_agr.add_trace(go.Scatter(
        x=df_agr["Mes_ES"], y=df_agr["Horas_Regulares"], mode="lines",
        name="Cap. Regular",
        line=dict(color=C["lavender"], dash="dot", width=2)
    ))
    fig_agr.update_layout(
        **PLOT_CFG, barmode="stack", height=400,
    ...
    with col1:
        ...
        fig_fl.add_trace(go.Bar(
            x=df_agr["Mes_ES"], y=df_agr["Contratacion"], name="Contrataciones",
            marker_color=C["mint"], marker_line_color="white", marker_line_width=1
        ))
        fig_fl.add_trace(go.Bar(
            x=df_agr["Mes_ES"], y=df_agr["Despidos"], name="Despidos",
            marker_color=C["rose"], marker_line_color="white", marker_line_width=1
        ))
        fig_fl.update_layout(
            **PLOT_CFG, barmode="group", height=300,
    ...
    with col2:
        ...
        fig_ex.add_trace(go.Bar(
            x=df_agr["Mes_ES"], y=df_agr["Horas_Extras"], name="Horas Extra",
            marker_color=C["butter"], marker_line_color="white", marker_line_width=1
        ))
        fig_ex.add_trace(go.Bar(
            x=df_agr["Mes_ES"], y=df_agr["Backlog_HH"], name="Backlog",
            marker_color=C["rose"], marker_line_color="white", marker_line_width=1
        ))
    ...
    with st.expander("Ver tabla completa"):
        ...
        st.dataframe(
            df_show.style.format({c: "{:,.1f}" for c in df_show.columns if c != "Mes"})
...
with tabs[2]:
    ...
    for idx, p in enumerate(PRODUCTOS):
        ...
        fig_des.add_trace(go.Scatter(
            x=df_p["Mes_ES"], y=df_p["Demanda"], mode="lines+markers",
            line=dict(color=PROD_COLORS_DARK[p], dash="dash", width=1.8),
        ...
        if not mes_row.empty:
            fig_des.add_trace(go.Scatter(
                ...
                mode="markers", marker=dict(size=16, color=C["coral"], symbol="star"),
                showlegend=False
            ), row=r, col=c)

    fig_des.update_layout(**PLOT_CFG, height=700, barmode="group",
    ...
    with col_cob1:
        ...
        fig_cob.add_vline(x=100, line_dash="dash", line_color=C["coral"],
                          annotation_text="Meta 100%", annotation_font_color=C["coral"])
    ...
    with col_cob2:
        df_cob = pd.DataFrame({
            "Producto": prods_c, "Producido": und_prod, "Demanda": und_dem, "Cob %": cob_vals
        })
        st.dataframe(
            df_cob.style.format({"Producido": "{:,.0f}", "Demanda": "{:,.0f}", "Cob %": "{:.1f}%"})
    ...
    for p in PRODUCTOS:
        fig_inv.add_trace(go.Scatter(
            ...
            line=dict(color=PROD_COLORS_DARK[p], width=2.5),
            marker=dict(size=7, color=PROD_COLORS[p], line=dict(color="white", width=2)),
            fill="tozeroy", fillcolor=hex_rgba(PROD_COLORS[p], 0.15)
        ))
    fig_inv.update_layout(**PLOT_CFG, height=300, xaxis_title="Mes", yaxis_title="Unidades",
...
with tabs[3]:
    ...
    st.markdown('<div class="info-box">SimPy · Rutas: Mezclado → Dosificado → Horneado → Enfriamiento → Empaque · Tiempos estocásticos.</div>', unsafe_allow_html=True)
    ...
    for i, (p, u) in enumerate(plan_mes.items()):
        with cols_p[i]:
            ...
            st.markdown(f"""
            <div class="kpi-card" style="background:{PROD_COLORS[p]}25;border-color:{PROD_COLORS_DARK[p]}50">
    ...
    if not df_kpis.empty:
        ...
        for i, row in df_kpis.iterrows():
            ...
            fig_cum.add_trace(go.Bar(
                ...
                marker=dict(color=PROD_COLORS[p_key], line=dict(color=PROD_COLORS_DARK[p_key], width=2)),
                text=f"{row['Cumplimiento %']:.1f}%", textposition="inside",
                textfont=dict(color="#3D2B1F", size=12), showlegend=False,
            ))
        ...
        with col_t1:
            ...
            fig_tp.update_layout(**PLOT_CFG, height=280, yaxis_title="und/h", showlegend=False,
                                 xaxis=dict(showgrid=False), yaxis=dict(gridcolor="#F0E8E0"), margin=dict(t=40))
        ...
        with col_t2:
            ...
            fig_lt.update_layout(**PLOT_CFG, height=280, yaxis_title="min/lote", showlegend=False,
                                 xaxis=dict(showgrid=False), yaxis=dict(gridcolor="#F0E8E0"), margin=dict(t=40))
    ...
    if not df_util.empty:
        ...
        if not cuellos.empty:
            for _, row in cuellos.iterrows():
                st.markdown(
                    f'<div class="pill-warn">Cuello de botella: <b>{REC_LABELS.get(row["Recurso"], row["Recurso"])}</b> — '
        ...
        fig_util_g.add_trace(go.Bar(
            x=rec_lb, y=df_util["Utilizacion_%"], marker_color=col_util,
            marker_line_color="white", marker_line_width=2,
            text=[f"{v:.0f}%" for v in df_util["Utilizacion_%"]],
            textposition="outside", showlegend=False
        ), row=1, col=1)
        fig_util_g.add_trace(go.Bar(
            x=rec_lb, y=df_util["Cola Prom"], marker_color=C["lavender"],
            marker_line_color="white", marker_line_width=2,
            text=[f"{v:.2f}" for v in df_util["Cola Prom"]],
            textposition="outside", showlegend=False
        ), row=1, col=2)
        fig_util_g.add_hline(y=80, line_dash="dash", line_color=C["coral"],
    ...
    if not df_lotes.empty:
        ...
        fig_gantt.update_layout(
            **PLOT_CFG, barmode="overlay", height=max(380, n_gantt*8),
        ...
        for p in PRODUCTOS:
            ...
            fig_violin.add_trace(go.Violin(
                y=sub_v, name=PROD_LABELS[p], box_visible=True,
                meanline_visible=True, fillcolor=PROD_COLORS[p],
                line_color=PROD_COLORS_DARK[p], opacity=0.85
            ))
        fig_violin.update_layout(**PLOT_CFG, height=320, yaxis_title="Tiempo en sistema (min)",
...
with tabs[4]:
    ...
    if not df_sensores.empty:
        ...
        s4.metric("⚠️ Excesos >200°C", excesos,
                  delta="Revisar horno" if excesos else "Normal",
                  delta_color="inverse" if excesos else "off")

        fig_temp = go.Figure()
        fig_temp.add_hrect(y0=150, y1=200, fillcolor=hex_rgba(C["mint"], 0.2), line_width=0,
                           annotation_text="Zona óptima", annotation_font_color=C["sage"])
        fig_temp.add_trace(go.Scatter(
            x=df_sensores["tiempo"], y=df_sensores["temperatura"],
            mode="lines", name="Temperatura", fill="tozeroy",
            fillcolor=hex_rgba(C["peach"], 0.15),
            line=dict(color=C["coral"], width=2)
        ))
        if len(df_sensores) > 10:
            mm = df_sensores["temperatura"].rolling(5, min_periods=1).mean()
            fig_temp.add_trace(go.Scatter(
                x=df_sensores["tiempo"], y=mm, mode="lines",
                name="Media móvil", line=dict(color=C["rose"], width=2.5, dash="dot")
            ))
        fig_temp.add_hline(y=200, line_dash="dash", line_color="#C0392B",
                           annotation_text="Límite 200°C", annotation_font_color="#C0392B")
        fig_temp.update_layout(**PLOT_CFG, height=320, xaxis_title="Tiempo (min)", yaxis_title="°C",
        ...
        with col_s1:
            ...
            fig_ocup.add_trace(go.Scatter(
                ...
                mode="lines", fill="tozeroy", fillcolor=hex_rgba(C["sky"], 0.3),
                line=dict(color=PROD_COLORS_DARK["Mantecadas"], width=2),
                name="Ocupación"
            ))
            fig_ocup.add_hline(y=cap_horno, line_dash="dot", line_color=C["coral"],
                               annotation_text=f"Cap. máx: {cap_horno}")
            fig_ocup.update_layout(**PLOT_CFG, height=280, title="Ocupación del Horno",
        ...
        with col_s2:
            ...
            fig_hist.update_layout(**PLOT_CFG, height=280, title="Distribución de Temperatura",
...
with tabs[5]:
    ...
    ESC_ICONS = {
        "Base": "🏠", "Demanda +20%": "📈", "Demanda −20%": "📉",
        "Falla en horno": "⚠️", "Capacidad reducida": "⬇️", "Capacidad ampliada": "⬆️",
        "Doble turno": "🕐", "Optimizado": "🚀"
    }

    escenarios_sel = st.multiselect(
        "Selecciona escenarios", list(ESCENARIOS_DEF.keys()),
    ...
    if st.button("🚀 Comparar escenarios", type="primary"):
        ...
        st.dataframe(
            df_comp.style.format({c: "{:,.2f}" for c in num_cols})
                      .background_gradient(
        ...
        if len(df_comp) > 1:
            ...
            with col_e1:
                ...
                if "Cumplimiento %" in df_comp.columns:
                    ...
                    fig_ec.update_layout(**PLOT_CFG, height=300, yaxis_title="%", showlegend=False,
                                         ...
                                         yaxis=dict(gridcolor="#F0E8E0"), margin=dict(t=30, b=80))
            ...
            if len(cols_radar) >= 3:
                ...
                COLORES_R = [C["rose"], C["sky"], C["mint"], C["butter"],
                           C["lavender"], C["peach"], C["sage"], C["lilac"]]
                RGBA_R = [hex_rgba(C["rose"], 0.15), hex_rgba(C["sky"], 0.15),
                          hex_rgba(C["mint"], 0.15), hex_rgba(C["butter"], 0.15),
                          hex_rgba(C["lavender"], 0.15), hex_rgba(C["peach"], 0.15),
                          hex_rgba(C["sage"], 0.15), hex_rgba(C["lilac"], 0.15)]
                    ...
                    **PLOT_CFG, height=450,
                    polar=dict(
                        radialaxis=dict(visible=True, range=[0, 1],
                                       gridcolor="#E8E0D8", linecolor="#E8E0D8"),
                        angularaxis=dict(gridcolor="#E8E0D8")
                    ),
...
st.markdown("---")
st.markdown("""
<div style="text-align:center;color:#8B7355;font-size:0.82rem;
     font-family:Inter,sans-serif;padding:0.4rem 0 1.5rem">
  🥐 <b>Gemelo Digital — Panadería Dora del Hoyo</b> · Planeación Agregada · Simulación SimPy · Streamlit
</div>""", unsafe_allow_html=True)

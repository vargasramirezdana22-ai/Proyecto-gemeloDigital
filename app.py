...
def fig_demanda():
    ...
    fig.update_layout(**THEME,barmode="group",
                      ...
                      legend=dict(bgcolor="rgba(255,255,255,0.8)",orientation="h",y=-0.28,x=0.5,xanchor="center"))
    return fig
...
def fig_utilizacion(df_u):
    ...
    fig.update_layout(**THEME,height=400,
                      title=dict(text="Utilización de Recursos — Detección Cuellos de Botella",
                                 x=0.5,font=dict(size=15,color="#E8A838",family="Barlow Condensed, sans-serif")))
    fig.update_xaxes(gridcolor="#e9ecef"); fig.update_yaxes(gridcolor="#e9ecef")
    return fig
...
def fig_kpis_radar(df_kpi):
    ...
    fig.update_layout(**THEME,height=420,
                      polar=dict(bgcolor="#f8f9fa",
                                 radialaxis=dict(visible=True,gridcolor="#dee2e6",tickcolor="#adb5bd"),
                                 angularaxis=dict(gridcolor="#dee2e6")),
                      title=dict(text="Radar de KPIs por Producto (normalizado)",
                                 x=0.5,font=dict(size=15,color="#E8A838",family="Barlow Condensed, sans-serif")),
                      legend=dict(bgcolor="rgba(255,255,255,0.8)",orientation="h",y=-0.15,x=0.5,xanchor="center"))
    return fig
...
def fig_sensores(df_s):
    ...
    fig.update_layout(**THEME,height=460,
                      title=dict(text="Sensores Virtuales — Monitor del Horno en Tiempo Real",
                                 x=0.5,font=dict(size=15,color="#E8A838",family="Barlow Condensed, sans-serif")))
    fig.update_xaxes(gridcolor="#e9ecef",title_text="Tiempo simulado (min)",row=2,col=1)
    fig.update_yaxes(gridcolor="#e9ecef",title_text="°C",row=1,col=1)
    fig.update_yaxes(gridcolor="#e9ecef",title_text="Estaciones",row=2,col=1)
    return fig
...
def fig_colas(df_u):
    ...
    fig.update_layout(legend=dict(bgcolor="rgba(255,255,255,0.8)",orientation="h",
...
def fig_comparacion(resultados_esc):
    ...
    fig.update_layout(**THEME,height=520,
                      title=dict(text="Comparación de Escenarios What-If",
                                 x=0.5,font=dict(size=15,color="#E8A838",family="Barlow Condensed, sans-serif")))
    fig.update_xaxes(gridcolor="#e9ecef",tickangle=25)
...
def kpi_card(titulo, valor, unidad="", color="#E8A838", icon="◈"):
    return html.Div([
        html.Div(icon+" "+titulo, style={**LABEL_STYLE,"color":"#6c757d"}),
        html.Div([
...
def tabla_dash(df, id_tabla, page_size=12):
    if df.empty: return html.Div("Sin datos", style={"color":"#6c757d"})
    return dash_table.DataTable(
        ...
        style_header={"backgroundColor":"#f8f9fa","color":"#E8A838",
                       "fontFamily":"IBM Plex Mono, monospace","fontSize":"10px",
                       "border":"1px solid #222","letterSpacing":"0.08em"},
        style_cell={"backgroundColor":"#ffffff","color":"#343a40",
                     "fontFamily":"IBM Plex Mono, monospace","fontSize":"11px",
                     "border":"1px solid #dee2e6","padding":"6px 10px","textAlign":"right"},
...
sidebar = html.Div([
    ...
    html.Div(id="sidebar-status", style={"padding":"16px","marginTop":"auto",
                                          "borderTop":"1px solid #dee2e6","position":"absolute",
                                          "bottom":"0","width":"100%"}),
...
panel_config = html.Div([
    html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dbc.Checklist(
                    ...
                    value=[], switch=True,
                    style={"color":"#343a40","fontSize":"12px",
                           "fontFamily":"IBM Plex Mono, monospace"},
                ),
            ], width=3),
        ]),
        ...
        dbc.Row([
            ...
            dbc.Col([
                html.Div(id="run-status",style={"color":"#198754","fontSize":"12px",
                                                 "fontFamily":"IBM Plex Mono, monospace",
                                                 "padding":"10px 0"}),
            ], width=9),
        ]),
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div(id="header-tab-name",
                   ...
                   "marginBottom":"0","background":"#f8f9fa"}),
...
def _no_data_msg():
    return html.Div([
        html.Div("◈", style={"fontSize":"48px","color":"#dee2e6","textAlign":"center","padding":"40px 0 8px"}),
...
try:
    ...
    print(f"✅ Dashboard listo en: {_url}")
